...
export default function Page({params}: {params : {id: string}}) {
    ...
    useEffect(() => {
        ...
        setData({...data, content: {list}, categoryPk: 2, title: '', writer: ''});
        setIsView(false);
    },[params.id]);
    ...
    const onChangeBlockHandler = useCallback((e: ChangeEvent<HtmlElements>, seq: number) => {
        ...
        const newList = data?.content?.list.map((item, index) => {
            if (item.seq === seq) {
            ...
            <div className={'w-full lg:w-2/3 xl:w-1/2 flex flex-col gap-6 duration-500'}>
                <div className={'flex justify-between gap-2 h-auto border-b-4 border-solid border-blue-200 py-3'}>
                    <div className={'font-bold flex items-center w-full'}>
                        ...
                        {
                            !isNewBoard && isView &&
                            <span className={'w-full text-lg py-2'}
                            >{data.title}</span>
                        }
                    </div>
                    {
                ...
                <div className={'flex flex-col'}>
                    {
                        ...
                        && data.content.list.map((item, index) =>
                            <Block key={index}
                                   ...
                                   openMenuToggle={({code}) => {openMenuToggle(item.seq, code)}}
